		...
		if(empty($_GET['action']) && php_sapi_name()!='cli') {
			...
			<div class="center-idea-container">
				...
				<p style="font-size:14px;" class="how-to-use-guide">
					Press <strong>X</strong> or your left arrow key to dislike, press â¤ï¸ or your right arrow key to like the idea
				</p>
			</div>
			...
			<a name="solver"></a>
			<h2>
				ðŸ§ &nbsp; Ask for a product idea that solves a problem
			</h2>
...
");
	...
	// <send out emails>
			...
			foreach($thisWeeksTopIdeas as $idea) {
				...
				'<a href="https://ideasai.net/?e=1&action=upvote&id='.$idea['id'].'">â¤ï¸ Like this</a>'.
				"<br/><br/>\n\n";
				$i++;
			}
			...
			foreach($emailsToSend as $email) {
				...
				$sendGridMail = new \SendGrid\Mail\Mail(); 
				$sendGridMail->setFrom('no-reply@ideasai.net','IdeasAI');
				$sendGridMail->setSubject("Your weekly ideas from ðŸ’¡ IdeasAI");
				$sendGridMail->addTo($email['email'],$email['email']);
				$message="<strong>Here's your weekly ideas generated by GPT-3 from <a href=\"https://ideasai.net\">ðŸ’¡ IdeasAI</a>:</strong><br/><br/>
".$emailSponsorHTML."
".$weeklyTopIdeasList."
<br/>
...
If you don't want to get these weekly ideas anymore, <a href=\"https://ideasai.net/?action=stop_receiving&email=".urlencode($email['email'])."&hash=".$hash."\">click here</a>";
			...
			if($sentEmails) {
				// sendToAdminTelegram("ðŸ’Œ Emailed ".number_format($sentEmails)." idea newsletters");
			}
	...
	// <voting logic>
			...
			if(!$alreadyVotedOnThis) {
				...
				// sendToAdminTelegram("ðŸ‘ ".$_SERVER['REMOTE_ADDR'].' '.$idea);
			}
			...
			if(!$alreadyVotedOnThis) {
				...
				// sendToAdminTelegram("âŒ ".$_SERVER['REMOTE_ADDR'].' '.$idea);
			...
			?><table style="max-width:700px;margin:14px auto;"><?
				...
				?><tr id="id_<?=$idea['id']?>" class="container"><?
					?><td class="td_idea">
						...
						<?/*<div class="action-upvote" data-idea-type="<?=$ideaType?>" data-solo="<?=$soloIdea ? 'true' : '';?>" data-id="<?=$idea['id']?>" style="text-decoration:none;font-size:28px;">&nbsp;â–²&nbsp;</div>*/?>
						...
						<?/*<div class="action-downvote" data-idea-type="<?=$ideaType?>" data-solo="<?=$soloIdea ? 'true' : '';?>" data-id="<?=$idea['id']?>" style="text-decoration:none;font-size:28px;">&nbsp;â–¼&nbsp;</div>*/?>
